...
export default function Home() {
  ...
  const gameState = useQuery(
    ...
    session
      ? { gameId: session.gameId as never, playerId: session.playerId as never }
      : "skip",
  );
  ...
  }, [gameState?.question, previewRanges]);

  useEffect(() => {
    if (!gameState?.question || !gameState.questionOpen || !regexInput.trim()) {
      return;
    }
      ...
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
        ...
        {!session ? (
          <section className="border-2 border-black bg-white p-6">
            <h2 className="text-lg font-semibold uppercase">Join Game</h2>
            {kickedMessage ? (
              <p className="mt-3 text-xs uppercase">{kickedMessage}</p>
            ) : null}
            <form onSubmit={handleJoin} className="mt-6 flex flex-col gap-4">
              ...
              <button
                type="submit"
                className="border-2 border-black bg-[var(--primary)] px-4 py-3 text-sm font-semibold uppercase hover:bg-black hover:text-white"
              >
                Join
              </button>
              {statusMessage ? (
                <p className="text-xs uppercase">{statusMessage}</p>
              ) : null}
            </form>
          </section>
        ) : (
            ...
            {gameState?.question && gameState.game.status === "active" ? (
              <div className="mt-6 flex flex-col gap-6">
                <div className="flex items-center justify-between text-base uppercase">
                  <span>Question: {gameState.question.index + 1}</span>
                  <span>Time: {timeLeft !== null ? `${timeLeft}s` : "--"}</span>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  <label className="text-base uppercase">Your Regex:</label>
                  <input
                    value={regexInput}
                ...
                {!gameState.questionOpen ? (
                  <div className="border-2 border-black bg-[var(--background)] p-4">
                    {gameState.playerRank ? (
                      <p className="text-xl font-semibold uppercase">
                        Your place: {gameState.playerRank} of{" "}
              ...
              <div className="mt-6 border-2 border-black bg-[var(--background)] p-4">
                {gameState.playerRank ? (
                  <p className="text-xl font-semibold uppercase">
                    Your final place: {gameState.playerRank} of{" "}
                    {gameState.leaderboard.length}
                  </p>
                ) : (
                  <p className="text-xl uppercase">No final rank.</p>
                )}
              </div>
            ) : (
