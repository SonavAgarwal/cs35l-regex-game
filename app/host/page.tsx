...
export default function RunnerPage() {
  ...
  useEffect(() => {
    ...
    const end =
      gameState.game.questionStartedAt + gameState.question.timeSeconds * 1000;
    const tick = () => {
  ...
  const parseQuestions = (text: string) => {
    ...
    if (idxTarget === -1 || idxAnswer === -1 || idxTime === -1) {
      return {
        questions: [],
        error:
          'Missing required columns: "target_string", "answer_regex", "time".',
      };
    }
          ...
          <header className="border-2 border-black bg-white px-6 py-4">
            <div className="flex flex-col items-center gap-4">
              <div className="text-4xl font-semibold uppercase">
                {gameCode ? `Game Code: ${gameCode}` : "Create Game"}
              </div>
            </div>
          </header>
        ) : null}
        ...
        {!gameId ? (
          <section className="border-2 border-black bg-white p-6">
            <button
              ...
              className="border-2 border-black bg-[var(--primary)] px-4 py-3 text-xl font-semibold uppercase hover:bg-black hover:text-white"
            >
              Create Game
            </button>
          </section>
          ...
          <section className="border-2 border-black bg-white p-6">
            <div className="flex flex-col gap-4">
              <label className="text-base uppercase">Questions CSV</label>
              <div className="flex flex-wrap items-center gap-3">
                <label className="border-2 border-black bg-[var(--primary)] px-4 py-2 text-xl font-semibold uppercase hover:bg-black hover:text-white">
                  Choose File
                  <input
                    type="file"
                    accept=".csv,text/csv"
              ...
              {csvError ? (
                <p className="text-base uppercase">{csvError}</p>
              ) : null}
              {uploadStatus ? (
                <p className="text-base uppercase">{uploadStatus}</p>
              ) : null}
            </div>
            {questions.length > 0 ? (
              <div className="mt-6">
                <p className="text-base uppercase">Question Preview</p>
                <div className="mt-3 flex flex-col text-base uppercase">
                  {questions.map((question, index) => (
                    ...
                    >
                      ...
                      <span className="text-sm">{question.answerRegex}</span>
                      <span className="text-sm">{question.timeSeconds}s</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              <button
                ...
                onClick={handleStart}
                disabled={!canStart}
                className="border-2 border-black bg-[var(--primary)] px-4 py-3 text-xl font-semibold uppercase disabled:cursor-not-allowed disabled:opacity-50 hover:bg-black hover:text-white"
              >
                Start Game
              </button>
              {gameId ? (
            ...
            {gameState?.leaderboard.length ? (
              <div className="mt-6">
                <p className="text-base uppercase">Joined Players</p>
                <div className="mt-3 flex flex-col text-base uppercase">
                  {gameState.leaderboard.map((entry) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => handleKick(entry.playerId)}
              ...
              <table className="mt-3 w-full border-2 border-black text-base uppercase">
                ...
                <tbody>
                  {gameState.leaderboard.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="px-3 py-3 text-left bg-white">
                        No players yet.
                      </td>
                    </tr>
                  ) : (
                    gameState.leaderboard.map((entry) => (
                      <tr key={entry.playerId} className="bg-white">
          ...
          <section className="border-2 border-black bg-white p-6">
            ...
            <div className="mt-4">
              ...
              <div className="mt-2 border-2 border-black bg-white">
                <div
                  className="h-6 bg-[var(--primary)] transition-all"
                  style={{
                    width: `${
                      maxTimeLeft
                        ? Math.max(
                            0,
                            Math.min(1, (timeLeft ?? 0) / maxTimeLeft),
                          ) * 100
                        : 0
                    }%`,
                  }}
